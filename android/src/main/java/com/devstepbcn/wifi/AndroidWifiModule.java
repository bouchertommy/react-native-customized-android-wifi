...
public class AndroidWifiModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void connectWithoutPasswordToWifi(String SSID, final Callback networkAdded) {
        try {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
                connectWithoutPasswordToWifiApiLowerThan29(SSID, networkAdded);
            } else {
    ...
    @ReactMethod
    private void connectWithoutPasswordToWifiApiLowerThan29(String networkSSID, Callback networkAdded) {
        ...
        List<android.net.wifi.WifiConfiguration> list = wifi.getConfiguredNetworks();
        if (list == null) {
            networkAdded.invoke(false);
            return;
        }

        for (android.net.wifi.WifiConfiguration wifiConfig : list) {
